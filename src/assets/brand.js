...
// The apliteni-ui KIT mark — a rounded "sub-theme prism" of the four ready-made
// accents (Nebula / Phoenix / Ocean / Emerald). Distinct from the Apliteni
// company seedling above: use the prism for the kit's OWN identity (the intro,
// the landing, the favicon); use the seedling for product lockups (Strategy,
// sign-in, consent). `p` prefixes the clip id so multiple marks never collide.
...
// Brand lockup: mark + word. `word` defaults to "Strategy" (the strategy portal).
export const brand = ({ p = 'lg', word = 'Strategy', size = 21, href = '/' } = {}) =>
  `<a class="brand" href="${href}" aria-label="Apliteni ${word}">${seedling(p, size)}<span>${word}</span></a>`;
