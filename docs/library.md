...
| `nav({ variant })`, dispatching to `sidebarNav`, `navTabs` or `breadcrumbs`, + `wireNav(root)` | Wayfinding. The umbrella dispatches on `variant`; each shape is also exported on its own. `wireNav` only drives the sidebar's collapsible groups. `sidebarNav({ activeIs: 'section' })` marks the active row as the current section (`aria-current="true"`) rather than the page, for a page that sits below that row. |
...
| `statBand({ stats, variant, basis, label, id })`, and `stat({ label, value, delta, trend })` for one figure, with `STAT_VARIANTS` and `STAT_TONES` | A row of key figures, as a `<dl>`. Each figure is `{ label, value, delta, trend }`; `delta` is `{ value, tone, basis, direction, none }`, where `tone` (`good`, `bad`, `neutral`) says whether the change is good news and is never read off its sign. `basis` is printed once under the band and every change points at it. `variant` is `band` (one card), `tiles` (a card per figure) or `open` (no surface). `trend` is your `<svg>`; the kit draws no chart. See [Stat bands](specification.md#stat-bands). |
