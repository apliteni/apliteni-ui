...
| `pagination({ page, pageSize, total, hasMore, pageSizes, variant, label, loading, href, id })`, with `PAGE_SIZES` and `DEFAULT_PAGE_SIZE` | The strip under a table or a list. It renders a page the caller has already computed — no rows go in — so a page counted by a server and one sliced out of an array produce the same markup. `variant` is `steps`, `numbered` or `jump`; `total: null` is the cursor-API shape and draws Prev and Next only, taking its end from `hasMore`. A control at an end is disabled, never removed. One page of content draws no steps at all, and with no `pageSizes` on offer it renders nothing. `PAGE_SIZES` and `DEFAULT_PAGE_SIZE` are the kit's page-size scale — read both from here rather than writing the numbers at a call site. See [Pagination](specification.md#pagination). |
