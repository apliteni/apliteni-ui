...
See [Vanilla HTML boundaries](specification.md#vanilla-html-boundaries) for the
complete text, URL, enum and trusted-HTML slot inventory and the rejected URL schemes.
...
| `dropdown({ label, value, variant, items, sections, foot, header, footer, align, direction, portal, scroll, search })` + `wireDropdown(root)`, with `dropdownMatch(label, query)` and `dropdownFiltering(query)` | Popover list. `variant: 'select'` renders a listbox and shows the value in the trigger; `'menu'` renders an action list. `search: true`, or `{ placeholder, label, empty, hint, query }`, pins a field above the rows that filters them as the reader types — see [A dropdown with a search field](specification.md#a-dropdown-with-a-search-field), and Guidelines / Component choice for when a list needs one. `direction` is `'down'` (default), `'up'` for a trigger with no room below it, or `'auto'` to let the wiring measure and flip. `portal: true` mounts the panel at the top of the tree its trigger is in — the page's `<body>`, a frame's own `<body>`, or an open shadow root itself — as `position: fixed`; it crosses none of those boundaries, and `wireDropdown(root)` registers the close handlers in that root's own document, so a kit drawn in a frame or a shadow root keeps its stylesheet, its viewport and its Escape. This is **the answer for a dropdown inside `.ui-app__rail`**, which is `position: sticky` with `overflow-y: auto` and so both clips the panel and seals its `z-index` in. See [The dropdown panel](specification.md#the-dropdown-panel). `foot` draws a block pinned to the panel's bottom edge, as `.ui-dropdown__foot`; `.ui-dropdown__head` is its mirror at the top, written as the page's own markup through the unwrapped `header` slot. Both bleed back through the panel's padding to the edge they sit on, so each one's rule and line run the full width, and the padding they read is `--ui-dropdown-pad` on `.ui-dropdown__panel` rather than a number a page has to copy. `footer` stays the unwrapped slot at the bottom and sits inside the drawn foot. What goes in a head or a foot is the page's own markup and its own layout — with one constraint, which is ARIA: a menu panel takes menuitems and a listbox panel takes options, so a control belongs in a foot only when `search: true` has made the panel a dialog. A title, a count or a note is at home in any of the three. A row is emitted as a `<div>`, or an `<a>` when the item carries `href`; a page writing its own rows may use a `<button>` instead, and `.ui-dropdown__item` resets whichever tag it gets so the three render the same — see [A dropdown row is a div, a link or a button](specification.md#a-dropdown-row-is-a-div-a-link-or-a-button). `dropdownMatch(label, query)` is the match itself — a substring anywhere in the label, ignoring case and accents — and `dropdownFiltering(query)` says whether a query narrows anything at all; both are published so a second implementation of this dropdown asks the kit rather than writing a second fold table. **React:** `<Dropdown>` renders this markup class for class and carries this keyboard, `search` included, with a `row` render prop so a router link can be the row; it has no `portal`, so a panel inside `.ui-app__rail` stays with the factory, and no `foot`. See [react/README.md](../react/README.md#dropdown). |
| `tooltip({ label, value, detail, placement, open, x, y, id })` + `wireTooltip(root)`, with `showTooltip(host, mark)` and `hideTooltip(host)` | The hover readout over a chart, a sparkline or any mark whose value is read by pointing at it. Render one inside a `.ui-tip-host` `[data-tip-host]` beside the marks (the wiring positions a host without the class; a host driven only by `showTooltip` needs it); a mark is any element carrying `data-tip-value`, with `data-tip-label` and `data-tip-detail` beside it, and a `[data-tip-anchor]` inside it places the readout on a dot rather than on the whole slice. The readout is absolutely placed, opens above the mark and flips below only when above is clipped, so showing it never moves the page. `showTooltip` is for a chart that does its own hit-testing, and Escape dismisses a readout whichever of the two showed it. Under a coarse pointer there is no hover to rest: a tap on a mark opens the readout, a tap on the same mark or anywhere else closes it, and the tap that opens it never reaches the mark's own `click`. See [The hover readout](specification.md#the-hover-readout). |
...
| `backLink({ href, label })` | The way up from a page to the page it sits under — a record back to its list. An `<a href>` to the address given, never a step through the history: no address, or a `javascript:` one, renders nothing. `label` is the destination as the sidebar spells it, and the link's name is "Back to" that label. In a page, hand it to `appShell()` as `back` rather than placing it yourself. See [The back link](specification.md#the-back-link). **React:** `<BackLink>` is the same link with the same rules, rendered rather than interpolated — which is what keeps `ui-back` on the element `.ui-app__main > .ui-back` looks for. See [react/README.md](../react/README.md#backlink). |
...
| `accountShell({ word, account, active, cap, crumb, title, sub, body, nav, versions, showSwitch, signOutHref, layout, width, search, collapsible, collapsed })` | **The compatibility preset for existing `/account` pages — not for new work; call `appShell()`.** It is `appShell()` with the topbar switched on, `ACCOUNT_NAV` as the nav it falls back to, and `#logout` as the sign-out. `title`, `sub` and `body` are its parent's trusted-markup slots, unchanged. `nav` takes the old `[id, icon, label, href?, target?]` tuples as well as `sidebarNav()` item objects. `cap` and `crumb` are text, and become the breadcrumb trail the shell used to write for itself. `collapsible`, `collapsed`, `layout`, `width` and `search` pass straight through to `appShell()` — and under `layout: 'topbar'` the preset's own topbar, with its version switcher and theme toggle, is not drawn, because that layout draws a band of its own. |
