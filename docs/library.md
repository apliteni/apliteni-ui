...
| `appShell({ word, brandHref, nav, active, navLabel, crumbs, title, sub, body, account, signOutHref, topbar, maxWidth })` | The kit's one page shell: a full-height rail built from `sidebarNav()`, beside exactly one `<main>`. `crumbs` is the caller's — pass `[{ label, href? }]` and the shell renders `breadcrumbs()`; pass nothing and there is no trail. `topbar` is off unless you hand it an options object, and turning it on drops the rail's brand head so the product word is said once. `signOutHref` adds the rail's sign-out row. Below 720px the rail folds to icons rather than disappearing. |
| `accountShell({ word, account, active, cap, crumb, title, sub, body, nav, versions, showSwitch, signOutHref })` | The `/account` preset over `appShell()`: the topbar switched on, `ACCOUNT_NAV` as the nav it falls back to, and `#logout` as the sign-out. `nav` takes the old `[id, icon, label, href?, target?]` tuples as well as `sidebarNav()` item objects. `cap` and `crumb` become the breadcrumb trail the shell used to write for itself. |
