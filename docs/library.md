...
| `dropdown({ label, value, variant, items, sections, head, foot, header, footer, align, direction, portal, scroll, search })` + `wireDropdown(root)` | Popover list. `variant: 'select'` renders a listbox and shows the value in the trigger; `'menu'` renders an action list. `search: true`, or `{ placeholder, label, empty, hint, query }`, pins a field above the rows that filters them as the reader types — see [A dropdown with a search field](specification.md#a-dropdown-with-a-search-field), and Guidelines / Component choice for when a list needs one. `direction` is `'down'` (default), `'up'` for a trigger with no room below it, or `'auto'` to let the wiring measure and flip. `portal: true` mounts the panel at the top of the tree its trigger is in — the page's `<body>`, a frame's own `<body>`, or an open shadow root itself — as `position: fixed`; it crosses none of those boundaries, and `wireDropdown(root)` registers the close handlers in that root's own document, so a kit drawn in a frame or a shadow root keeps its stylesheet, its viewport and its Escape. This is **the answer for a dropdown inside `.ui-app__rail`**, which is `position: sticky` with `overflow-y: auto` and so both clips the panel and seals its `z-index` in. See [The dropdown panel](specification.md#the-dropdown-panel). `head` and `foot` pin a block to the panel's top or bottom edge, drawn as `.ui-dropdown__head` and `.ui-dropdown__foot`: each bleeds back through the panel's padding to the edge, so its rule and its line run the full width, and the padding it reads is `--ui-dropdown-pad` on `.ui-dropdown__panel` rather than a number a page has to copy. `header` and `footer` stay the unwrapped slots and sit inside the drawn blocks. What goes in a head or a foot is the page's own markup and its own layout — with one constraint, which is ARIA: a menu panel takes menuitems and a listbox panel takes options, so a control belongs in a foot only when `search: true` has made the panel a dialog. A title, a count or a note is at home in any of the three. A row is emitted as a `<div>`, or an `<a>` when the item carries `href`; a page writing its own rows may use a `<button>` instead, and `.ui-dropdown__item` resets whichever tag it gets so the three render the same — see [A dropdown row is a div, a link or a button](specification.md#a-dropdown-row-is-a-div-a-link-or-a-button). |
