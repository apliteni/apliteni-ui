...
| `pager({ page, perPage, total, hasMore, rowsOnPage, tier, perPageOptions, href, label, busy })` + `pagerRange({ page, perPage, total, hasMore, rowsOnPage })` | Table pagination, in three tiers — `compact` (range + prev/next), `advanced` (+ rows-per-page, jump, first/last) and `numbered` (a truncated page list). The indicator is a row range, `1–100 of 4,812`, and a query that never paid for a total says `1–100 of more than 100` — resolving to the exact figure on the last page, where it is arithmetic. `numbered` throws on an unknown total rather than draw a last page that does not exist. `href` makes every control a real `<a>` that works with no JavaScript; without it they are real `<button>`s the consumer wires, keyed by `data-page`. `pagerRange` is the sentence on its own, for a page that wants the position without the control. In `advanced`, `perPageOptions` defaults to `[10, 25, 50, 100]`, and a page size the caller is already on but never offered joins that list rather than reading as unselected. `busy` disables every control and leaves the range on screen. A single page renders nothing except in `advanced`, where the rows-per-page control is reason enough to stay, and no rows renders nothing in every tier — the empty state belongs to the table, not to the control under it. Which tier answers which table is the `Tables at scale` page in the Guidelines collection; what the control guarantees is [Table pagination](specification.md#table-pagination). |
