...
| `tooltip({ label, value, detail, placement, open, x, y, id })` + `wireTooltip(root)`, with `showTooltip(host, mark)` and `hideTooltip(host)` | The hover readout over a chart, a sparkline or any mark whose value is read by pointing at it. Render one inside a `.ui-tip-host` `[data-tip-host]` beside the marks; a mark is any element carrying `data-tip-value`, with `data-tip-label` and `data-tip-detail` beside it, and a `[data-tip-anchor]` inside it places the readout on a dot rather than on the whole slice. The readout is absolutely placed, opens above the mark and flips below only when above is clipped, so showing it never moves the page. `showTooltip` is for a chart that does its own hit-testing. See [The hover readout](specification.md#the-hover-readout). |
