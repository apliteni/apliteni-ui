...
const swatch = (name, note = '') => `
  <div style="display:flex;flex-direction:column;gap:8px">
    ...
    <div>
      <div style="font:600 12.5px/1.3 var(--font-sans);color:var(--strong)">${name}</div>
      ${note ? `<div style="font:400 11.5px/1.4 var(--font-sans);color:var(--muted)">${note}</div>` : ''}
    </div>
  </div>`;
...
export const Semantic = {
  render: () => pad(`
    <h1 style="font:700 30px/1.1 var(--font-display);color:var(--strong);letter-spacing:-.02em;margin-bottom:8px">Semantic colours</h1>
