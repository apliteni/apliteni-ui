...
export const Ladder = {
  render: () => pad(`
    ...
    ${p('<strong style="color:var(--strong)">In light the top step is the exception.</strong> Switch the theme on this page and the last swatch goes the other way: <code style="font-family:var(--font-mono);color:var(--accent)">--surface-3</code> is <code style="font-family:var(--font-mono);color:var(--accent)">#e7eaf1</code>, below the page and a hair above the sunken step, because it cannot be above a floating panel that is already white. In light it means the quiet fill — a chip, a hovered row, the readout’s panel — and on its fill alone a light readout would read as recessed rather than raised. The readout’s drop and its two-step edge are what say <em>raised</em> in light, where its fill cannot. Open on <a href="https://github.com/apliteni/apliteni-ui/issues/295" style="color:var(--accent)">#295</a>.')}
