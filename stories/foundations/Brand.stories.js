import { seedling, brand } from '../../src/assets/brand.js';
...
export const Mark = {
  render: () => pad(`
    ...
    <p style="color:var(--dim);margin-bottom:36px">The Apliteni seedling — a green-to-lime gradient mark. Pair with a product word.</p>
    <div style="display:flex;gap:40px;align-items:flex-end;flex-wrap:wrap;margin-bottom:44px">
      ${[64, 40, 28, 20].map((s) => `<div style="display:flex;flex-direction:column;align-items:center;gap:10px">${seedling('m' + s, s)}<code style="font-family:var(--font-mono);font-size:11px;color:var(--muted)">${s}px</code></div>`).join('')}
    </div>
