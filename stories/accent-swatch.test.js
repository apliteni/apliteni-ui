 ...
 *
 * The selector is matched the way stories/lib/contrast.js:56-60 `const wanted = [`
 * matches it —
...
function requireOwnDarkBlock(accent) {
  ...
  assert.ok(
    ...
    + '— because that is the only form stories/lib/contrast.js:56-60 `const wanted = [` resolves. '
    + 'Written any other '
...
function swatchesIn(file, html) {
  ...
  for (const [, attrs, inner] of html.matchAll(BUTTON)) {
    ...
    assert.ok(
      ...
      + '— inlining the {{FOOTER}} placeholder at site/index.html:376 `{{FOOTER}}` would put one '
      + 'there — then '
