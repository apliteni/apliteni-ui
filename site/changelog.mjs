...
export const RELEASES = [
  {
    ...
    changes: [
      ['breaking', "React `DataTable` draws no pager unless you pass `pager`, and pages at 25 rows when it does rather than 4. It used to render `.rx-pager` unconditionally \u2014 a client pager over whatever rows it was handed, reading `Page 1 of 1 \u00b7 N rows` \u2014 which is false under a page a server has already paged, and the only way to be rid of it was to hide it with CSS. If you relied on the old pager, pass `pager`. If you set `pageSize` only to stop it paging a server's page a second time, drop the prop. Nothing emits `.rx-pager` any more, so a rule of yours that hid it now matches nothing and can be deleted.", ['DataTable']],
      ['added', "React `<Pagination>` draws the same control as `pager()` over the same stylesheet, in all three tiers and both total shapes, and imports the range sentence rather than writing a second one. `renderLink` hands each control to your router's link, `focusRef` moves focus onto the rows a turned page describes, and the range announces itself only in button mode, because a link navigates and the page load already says so.", ['Pagination', 'DataTable']],
