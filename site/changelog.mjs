...
export const RELEASES = [
  ...
  },
  {
    ...
    changes: [
      ...
      ['fixed', "The `<select>` chevron now follows the theme’s `--muted` color. Its stroke was a hexadecimal value frozen inside a data URI — the dark `--muted` value from before this release — and it had a 2.49:1 contrast ratio on the light field. Because a data URI cannot read a token, the ink is written separately for each theme.", ['Inputs']],
    ],
  },
  {
  ...
  },
  {
    ...
    changes: [
      ['added', "`backLink()` now provides a quiet link from a page to its parent page. It appears above the title where a breadcrumb trail would normally appear. It is an `<a href>` to an address named by the caller, never a step through browser history: a page opened in a new tab, from a bookmark, or from a shared address has no history to follow, and the browser’s Back button already handles history. The link shows the destination name beside an arrow and is announced to screen readers as \"Back to\" followed by that destination. Without a name, it shows \"Back\"; when a label already begins with \"Back to\", it is read as the place after those words. It uses `--dim` without a box, and its color rule outranks a host stylesheet’s `a:link`, so page-level link colors do not change it into another style. The kit provides only this treatment and no variant. Four treatments were rendered side by side on the review page for #270 — a quiet link, a bordered button above the title, an icon-only arrow beside the title, and the breadcrumb trail alone — and the quiet link was selected.", ['Back link']],
  ...
  },
  {
    ...
    changes: [
      ['breaking', "`drawer({ open: true })` now opens as an active drawer. The page behind becomes inert, Tab stays inside the panel, and Escape closes it. For a sidebar or inline specimen that should remain visibly open without this interaction, use `specimen: true`. `confirm()` interprets `open: true` in the same way."],
