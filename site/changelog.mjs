...
export const RELEASES = [
  {
    ...
    changes: [
      ['breaking', "Nothing in the kit casts a shadow. Every surface colour in both themes moved with it, so a screen built on the kit looks different the day it upgrades — the dark page is darker, the dark card and every floating panel are lighter, and the light page and card have come off white. A floating panel is the only pure white left on a light screen. Sixteen rules stopped drawing a `box-shadow`: the switch knob, the topbar theme pill, the soft toast, the success panel, the outline toast, the hover readout, the dropdown panel, the account menu, the workspace menu, the confirm, the drawer, the command palette, the auth card, the solid toast, the React modal and the active segmented pill. What separates a surface now is its step on the elevation ladder and the kit's hairline around it.", ['Card', 'Dropdown', 'Drawer', 'Confirm', 'CommandPalette', 'Modal', 'Tooltip', 'Callout', 'Topbar', 'Switch', 'Segmented']],
