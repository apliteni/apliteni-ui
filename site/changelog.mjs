...
export const RELEASES = [
  ...
  },
  {
    ...
    changes: [
      ['changed', 'Busy buttons slide the action label down and replace it with centered pulsing dots, then return the label from below. Every size and variant keeps its width while busy; reduced motion uses static dots. Resolves #327.', ['Button']],
      ['added', 'setButtonBusy updates vanilla buttons in place, retaining focus and icons while blocking activation and announcing progress. The helper takes only the busy state; React uses the same treatment without a public motion helper; explicit disabled state is preserved.', ['Button']],
    ],
  },
  {
  ...
  },
  {
    ...
    changes: [
      ['changed', "The hover readout answers a tap on a touch screen, where before it answered nothing a reader could use. A finger rests nowhere \u2014 it arrives already pressing and is gone when it lifts \u2014 so the readout flashed under the tap and went with it. Under a coarse pointer `pointerover` now opens nothing and the tap is the switch instead: a tap on a mark opens its readout, a tap on another mark moves it, and a tap on the same mark or anywhere else on the page closes it. The tap that opens the readout is captured and never reaches the mark's own `click`, so a chart that drills down on a bar does not drill down on the tap that asked what the bar says; the tap that closes the readout is let through, which puts the drill-down one tap further away. The document hears that opening tap all the same \u2014 it is handed on there, aimed at the root element \u2014 so a dropdown panel standing open closes under the tap that opens a readout as it would under any other tap. Which pointer is in play is read from the event rather than from the device, so a laptop with a touch screen hovers under its mouse and taps under a finger, and a keystroke hands the readout back to focus. A pen taps with that finger rather than hovering with the mouse: a stylus presses the screen rather than resting over it. Focus opens the readout under a coarse pointer too, unless it is the focus a tap lands on its way to the click that decides \u2014 so a reader stepping onto a mark through a screen reader gets the value and the `aria-describedby` that announces it, where a finger gets it from the tap. A tap that closes a readout dismisses that mark the way Escape does whether it landed on the mark or on the chart's ground beside it; a tap outside the host is the pointer leaving, and dismisses nothing. Keyboard focus opens it and Escape closes it as before, and this settles half of what #282 left open \u2014 whether a chart's marks should take a tab stop at all is still on that issue.", ['Tooltip']],
      ['changed', "Guidelines / Hover readouts gains a fifth rule: on a touch screen, open the readout with a tap and close it with the next one. The page no longer says a tap shows the readout only while the finger is down, which stopped being true with this release.", ['Tooltip']],
    ],
  },
  {
  ...
  },
  {
    ...
    changes: [
      ...
      ['added', "The content column comes in two widths. `appShell({ width: 'wide' | 'centered' })`, on both layouts: `centered` — the default, chosen on #308 — is the capped, centred column the kit has always drawn, and `wide` fills the track beside the rail. `maxWidth` is the number under either name.", ['Shell']],
      ['added', "The topbar layout's search opens the command palette. It is a trigger drawn as a field, carrying the palette's own `[data-cmdk-open]` hook and the key that opens it inside its accessible name — \"Search or run a command… Ctrl K\" — and `wireShell()` writes the reader's own platform into the key. Kept in the name rather than hidden, on #308: the shortcut is the one fact the control exists to teach.", ['Shell', 'CommandPalette']],
      ['added', "React: `<Dropdown>` — the React face of `dropdown()` and of `wireDropdown()`'s keyboard: both variants, sections, badges, separators, disabled and danger rows, controlled or uncontrolled `open`, `onSelect` and `onOpenChange`. A `row` render prop draws each row, so a router `<Link>` can be the row without losing its classes, role, tab stop or keyboard. `search` puts the kit's own field over the rows, filtering through the kit's own matcher. Held against the factory shape by shape, and against the factory-plus-`wireDropdown()` for what a typed query leaves standing. No `portal` yet, and no `foot` slot yet.", ['Dropdown']],
      ['added', "`dropdownMatch(label, query)` and `dropdownFiltering(query)` are exported from the kit: the match a dropdown's search field already used, published so a second implementation asks rather than writes a second fold table. The factory's own output is unchanged.", ['Dropdown']],
      ['added', "React: `<BackLink>` — the React face of `backLink()`, with the same rules and the same refusals, rendered rather than interpolated, which is what keeps `ui-back` on the element the shell's `.ui-app__main > .ui-back` rule looks for. Takes `as`, so a router link can be the anchor."],
      ['added', "`.ui-dropdown__panel` declares `--ui-dropdown-pad` beside `--ui-dropdown-gap` and pads itself with it, so a page pinning its own block inside the panel reads the number instead of copying `-6px` out of the kit's stylesheet. New `.ui-dropdown__foot`, the mirror of `.ui-dropdown__head`: the same inner padding from one shared rule, bleeding to the panel's bottom edge, with its line on the edge it faces. `dropdown({ foot })` draws it; `footer` remains the unwrapped slot and sits inside it. Foot only, by decision on #306: a head stays the page's own markup through `header`. A head or a foot holding a control belongs in a panel that may hold one — `search: true` makes the panel a dialog; a menu or a listbox takes rows and nothing else.", ['Dropdown']],
      ['added', "Type ranks — a sixth rank, caption: `--text-sm` at `--weight-normal`, for a sentence under a specimen, figure or screenshot. It is the first rank that does not take a size of its own: it shares the label's 13px and is separated from it by weight, so the rank table's order rule now reads \"smaller, or the same size and lighter\". Guidelines / The page had been borrowing the label rank for its captions, which put a medium 13px line under a normal 14.5px one; its captions are normal weight now. The gate that holds the ranks reads the rank notes in the stories, site pages, docs and React sources as well as in the sheets the kit ships — a rule drawn in a story is held by the same table."],
      ['fixed', "Back link — the destination's name clips instead of wrapping. `backLink()` has emitted `<span class=\"ui-back__label\">` since 0.29.0 with no rule behind it, so a parent page with a long name wrapped to two lines or four above the page title. It now takes one line and an ellipsis, the same way a rail row's label does, and `.ui-back` takes a `max-width` so the column is what stops it. A short name is unchanged. The full name stays in the accessible name. Chosen on #303 over wrapping like the breadcrumb trail, which on a phone pushed the page title down."],
      ['fixed', "A dropdown trigger's caret is centred on its ink rather than on its box, in both states. It is two borders of a rotated square, so the mark is an L and the corner opposite it is empty; the caret sat 1.75px low pointing down and 3.88px high pointing up, and now lands within a quarter pixel of the trigger's middle either way.", ['Dropdown']],
      ['fixed', "In light, a dropdown's search field takes `--bg` rather than `--surface-2`. A sunken box is read against the surface it sits in, and a floating panel is a rung above a card — in light that panel is pure white, so the ordinary field token fell two rungs under it and read as a hole.", ['Dropdown']],
      ['added', "A gate over every `__label` the kit emits. `src/styles/label-coverage.test.js` refuses a component that puts a `__label` into its markup with no rule for it in either workspace's CSS — the omission above, which a consumer's own class-coverage guard found first."],
    ],
  },
  {
    v: '0.33.1', date: '2026-09-13',
    changes: [
      ['changed', "A floating surface casts again, and only a floating one. What floats is decided by the surface's job rather than by its rung on the elevation ladder — a dropdown panel, the account and workspace menus, `confirm()`, the drawer, the React modal, the three toast styles, the command palette, and the hover readout, which paints `--surface-3`, the step above the rest of them. Each draws its hairline twice (`--border-strong` on the border, `--border` one pixel inside it) and adds one broad, faint drop under it. A surface that is merely high does not move: a card, a field, a chip and a hovered row — the last two sitting on the readout's own step — still say how high they are with their step and their line. Decided on #309 against the measured frames in `docs/reviews/295-popover-variants.html`: the panel was 1.05 above the card in light on a 1px line at 1.18, so the whole separation rested on one pixel. The edge now reads 1.64 in dark and 1.44 in light, and the drop's core reads 1.20 / 1.44 against the card.", ['Dropdown', 'Drawer', 'Confirm', 'Callout', 'Tooltip', 'CommandPalette', 'Topbar']],
      ['added', "`--elev-drop` — the drops of the floating treatment, one token per theme, in the order Primer's `--shadow-floating-*` uses: the inset 1px line first, then these. A floating surface writes the pair itself, `box-shadow: inset 0 0 0 1px var(--elev-edge, var(--border)), var(--elev-drop)`, and composes its focus ring in front of both. The line is written at the call site rather than inside the token because CSS substitutes a `var()` inside a custom property against the element that declares it: an `--elev-edge` read at `:root` resolves at `:root`, and no surface below could re-point it. `--elev-edge` is that hook, so a status toast draws its inner line in its own colour; unset it is `--border`. The five deprecated `--shadow-*` tokens are unchanged and still resolve to `0 0 #0000`.", ['Callout']],
      ['fixed', "The collapsed rail's flyout label read `--shadow-md` in both of its rules — a token deprecated in 0.32 that resolves to the transparent shadow — so it painted nothing while claiming to cast. It takes the floating treatment instead: it is the hover readout's twin, a readout placed over the page for as long as a pointer rests on a row, and it paints the same step. Its border firms to `--border-strong` with the rest of the floating step. A consumer who sets `--shadow-md` themselves gets the kit's drop on that label rather than their own shadow.", ['Shell']],
    ],
  },
  {
    ...
    changes: [
      ...
      ['breaking', "`footer()`'s column titles are `h2`, not `h4`. On a page they were the kit's only `h4` and the outline read `h2` → `h4`. A stylesheet that keyed on `.ui-footer h4` has to key on the class.", ['Footer']],
      ['changed', "`success()` gives its title the rank its layout earns — `h1` for `hero` and `split`, which are the page, `h2` for `compact` — where it was always an `h3`, so a success screen had no page title. `level` overrides it. The consent screen's \"Access granted\" is an `h1`; that page had no heading at all.", ['Success']],
      ['added', "The rail folds. `appShell()` draws a toggle at the far end of the brand row — one icon, a frame that holds still and a seam that crosses it — which folds the rail to the icon strip used below 720px and opens it again. The fold animates: the column keeps its open width and the box closes over it, so every glyph holds its place while the width travels and the words fade ahead of the closing edge; the toggle rides that edge onto the glyph column and the product's lockup goes with the fold. A folded row gives its name back beside the glyph on hover and on keyboard focus, drawn in CSS and pinned to the row where the browser has anchor positioning. `collapsible: false` draws no toggle; `collapsed` sets the first paint.", ['Shell']],
      ['added', "`wireShell(root, { persist })`, `railCollapsed(cookieHeader?)` and `RAIL_COOKIE`. `wireShell()` wires the toggle, the reader's menu and the nav's groups; a press is kept in the `apliteni-ui-rail` cookie and applied to any collapsible shell drawn without `collapsed`, and a server passes `railCollapsed(request.headers.cookie)` to paint the right width first. `persist: false` keeps every shell under `root` out of the cookie. Each press sends a bubbling `ui-rail` event with `detail.collapsed`.", ['Shell']],
      ['added', "The signed-in reader at the rail's foot opens a menu, with Sign out as a row of it. It is the kit's own `dropdown()`, portalled clear of the rail and opening upward; a destructive row turns `--pink` under the keyboard as well as under the pointer.", ['Shell', 'Dropdown']],
      ['added', "Guidelines / The page — what one screen may hold: the head's order, one title, an outline that stops three levels deep, one primary action, six cards, nothing overlaying the page at load, one density, a two-sentence lede. Two more limits — a page is `appShell()`'s, and every nav landmark is named — are in `docs/specification.md#the-page`, which maps every rule to the line of the kit that holds it. All ten are held by `stories/guidelines/the-page.test.js` over every screen in `stories/apps/`. The page itself is set in one ink, with its hierarchy carried by the type ranks rather than by grey."],
      ['fixed', "A dropdown panel is visible in the frame it opens rather than the next one, so the arrows put focus on a row instead of leaving it on the trigger with the next Tab stepping out of the menu; and it stops taking clicks the moment it begins to fade out rather than `--dur-med` later — in the account menu a stray click could sign the reader out of a menu nobody could see. Both hold for every menu the kit draws: `dropdown()`'s panel, the rail's portalled copy, and the topbar's version switcher and account menu.", ['Dropdown', 'Topbar']],
      ['fixed', "`sidebarNav({ collapsed })` no longer forces a group shut or hides its list, so the current page stays reachable on the folded rail. A row with no glyph gets a dot.", ['Shell']],
    ],
  },
  {
    v: '0.32.0', date: '2026-09-12',
    changes: [
      ['breaking', "Nothing in the kit casts a shadow. Every surface colour in both themes moved with it, so a screen built on the kit looks different the day it upgrades — the dark page and the dark card are darker, every dark floating panel is lighter, and the light page and card have come off white. A floating panel is the only pure white left on a light screen. Every rule that cast one stopped: the switch knob, the topbar theme pill, the soft toast, the success panel, the outline toast, the hover readout, the dropdown panel, the account menu, the workspace menu, the confirm, the drawer, the command palette, the auth card, the solid toast, the React modal, the active segmented pill, the floating feedback composer, the `.m-lift` hover utility, and the feedback pill — which cast an accent glow and a layer of ink at rest and grew both on hover, and whose glyph dropped a 1px ink `drop-shadow()` as well. What separates a surface now is its step on the elevation ladder and the kit's hairline around it; the pill takes neither, being the accent at full fill strength on the page.", ['Card', 'Dropdown', 'Drawer', 'Confirm', 'CommandPalette', 'Modal', 'Tooltip', 'Callout', 'Topbar', 'Switch', 'Segmented', 'Feedback']],
      ['breaking', "`--shadow-sm`, `--shadow-md`, `--shadow-lg`, `--shadow-seg` and `--shadow-card` are deprecated and resolve to the transparent shadow `0 0 #0000` in both themes. They are still published, so a consumer reading one gets nothing rather than an unresolved `var()`; a consumer who wants a cast shadow back sets them. Transparent rather than `none`, because a shadow token is read in a list: the kit's own pre-0.32 pattern was `box-shadow: var(--shadow-lg), var(--ring)`, and `none` inside a list makes the whole declaration invalid — the browser drops it, focus ring included. A consumer composing a kit shadow with anything else needs no migration; one who wants the surface bare should drop the `var()` rather than rely on it painting nothing. `--shadow-ink`, `--sheen`, `--ring` and `--scrim` are untouched — none of them is a cast shadow. The drawer, the confirm and the command palette also lose their local `--drawer-shadow`, `--confirm-shadow` and `--cmdk-shadow` hooks, which now have nothing to carry.", ['Drawer', 'Confirm', 'CommandPalette']],
      ['breaking', "The surface tokens are an ordered ladder, and three of them changed job. `--surface-2` is the SUNKEN step — a field, a track, a disabled box — so the six floating surfaces that painted it (the dropdown panel, the account and workspace menus, the confirm, the drawer, the command palette) paint `--bg-elevated` instead. `--bg-elevated` is the FLOATING step and is a full step above the card in dark, where it used to sit below it. Anything consuming these tokens for a purpose other than the one the ladder names has to be re-pointed. Dark: `--bg` #0e0d14, `--surface-2` #161520, `--surface` #211e2d, `--bg-elevated` #2a2639, `--surface-3` #2d293c, `--seg-active-bg` #383350. Light: `--bg` #eef0f5, `--surface-2` #e3e6ee, `--surface` #f8f9fc, `--bg-elevated` #ffffff, `--surface-3` #e7eaf1."],
      ['breaking', "`.ui-card` takes a hairline in BOTH themes, where only light had one. `.ui-card--accent` re-colours that hairline instead of drawing an inset ring inside it, so an accent card has one edge rather than two, and `.ui-card--interactive` no longer zeroes the border — a consumer that relied on a borderless dark card overrides `border`.", ['Card']],
      ['changed', "Dark `--muted` is lifted from #948fa8 to #a29db6 — the same violet-grey, fourteen steps up every channel. It is picked against the TOP of the ladder rather than against the page, because it carries a dropdown row's description and the readout's label and both of those ride the highest surfaces. It clears AA with room on every step the ladder raises, where the old value cleared the top two by hundredths. `--muted` is on every secondary line in the kit, so this is visible everywhere."],
      ['changed', "Light's signal inks came down a step with the page. `--pink` is #a92d59 and `--glow-pink` is re-tinted from it; `--chip-success-ink`, `--chip-warn-ink` and `--chip-info-ink` are deepened. Every one of them is read on a translucent wash, every wash now composites over a page that is no longer white, and the pairs went under WCAG AA where they had cleared it. Light Ocean's and light Emerald's `--accent` are deepened for the same reason.", ['Badge', 'Callout', 'Nav']],
      ['fixed', "In light, the hover readout gets its panel back. The kit re-pointed `--ui-tip-bg` at `--surface` in light, which was white while the page was white; the ladder makes `--surface` the card step, so a readout drawn over a card was painted the card's own colour and the panel disappeared. The override is gone and the readout takes `--surface-3` in both themes.", ['Tooltip']],
      ['fixed', "The accent counter in a dropdown row stops stacking its wash on the panel under it, and takes the badge's own flat surface with the accent as ink — the shape the nav badge has had since #157. A wash over a raised surface sits closer to the ink read on it than the same wash over the page, and the ladder made the panel a raised surface.", ['Dropdown']],
      ['fixed', "The resting glyph in the app rail is legible against the rail again. Its opacity was tuned when the light rail was one notch under white, and the ladder took the rail down with the page.", ['Nav']],
      ['fixed', "A hover inside a raised panel lifts instead of sinking. A hovered, focused or active dropdown row, the workspace and account menu rows, the drawer's close button, the dropdown's trailing badge and accent counter, the workspace menu's archive chip and the palette's key cap all painted `--surface` — the card step, which the ladder puts BELOW `--bg-elevated` in dark, so each of them sank while the panel it was in floated. They take `--surface-3`, the step above the panel, which is what the command palette's active row had always done. The dropdown's search field goes the other way and takes the sunken step `--surface-2`, the same one `.ui-input` takes.", ['Dropdown', 'Topbar', 'Drawer', 'CommandPalette']],
      ['fixed', "The switch knob has an edge again. `--shadow-sm` was the only thing drawing it, and with the shadow gone an off knob in light is a white disc on a `#e7eaf1` track at 1.20:1. It takes the `--border-strong` hairline the checkbox beside it already uses.", ['Switch']],
      ['fixed', "The chevron on a `<select>` follows the theme's `--muted`. Its stroke was a hex frozen into a data URI — dark `--muted`'s value from before this release — which read 2.49:1 on the light field. A data URI cannot read a token, so the ink is written once per theme.", ['Inputs']],
    ],
  },
  {
