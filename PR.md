...
| 7 | A citation pointed at `tokens.css:171` (`--disabled-ink-bare`) for `--disabled-ink` | Re-pointed to line 164, with the whole declaration as the pattern — the old one passed only because `--disabled-ink` is a prefix of `--disabled-ink-bare` |
